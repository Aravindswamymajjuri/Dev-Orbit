...
// Add new route to get logged in user's team
router.get('/myteam', authenticateToken, async (req, res) => {
  try {
    ...
    console.error("Error in /myteam route:", error.message);
...
router.get('/students/search', async (req, res) => {
  try {
    ...
    const teams = await Team.find({ students: { $in: studentIds } }).select('students');

    // Create a set of student IDs that are already in teams
    const studentsInTeams = new Set(teams.flatMap(team => team.students.map(id => id.toString())));
